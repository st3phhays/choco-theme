          ...
          const autoCompleteJS = new import_autocomplete.default({
            ...
            resultsList: {
              element: (list, data) => {
                ...
                if (getCookie2(templateHelpCookie)) {
...
<p class="mb-0 text-end autocomplete-downloads"><small>${data.value.DownloadCount} downloads</small></p>`;
  ...
  (() => {
    ...
    if (announcementCount && announcementCookie) {
      if (!getCookie2(announcementCookie.value)) {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    internalRepoUrls.forEach((el) => {
      if (getCookie2("internal_repo_url")) {
        for (let i = 0; i < internalRepoUrls.length; i++) {
          internalRepoUrls[i].value = getCookie2("internal_repo_url");
        }
      }
    ...
    const pageScriptType = () => {
      ...
      if (getCookie2("deployment_method") == "individual" || !getCookie2("deployment_method")) {
      ...
      if (installStepsGeneric) {
        if (getCookie2("deployment_method") == "chef" || getCookie2("deployment_method") == "puppet") {
