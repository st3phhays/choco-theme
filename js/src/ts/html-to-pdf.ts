...
export const htmlToPdf = (contentId: string, exportName: string) => {
    ...
    exportButton.forEach(button => {
        button.addEventListener('click', () => {
            ...
            const preCodeNodes = clonedHtml.querySelectorAll('.pdf-code-block');

            preCodeNodes.forEach(pre => {
                // Remove styling off the <pre> element and <code> element
                pre.setAttribute('style', 'width: 100%;');
                pre.querySelector('code').setAttribute('style', '');
            ...
            const converted = htmlToPdfmake(minifiedHtml, {
                defaultStyles: {
                    ...
                    pre: { font: 'CourierPrime' },
                    code: { font: 'CourierPrime' }
                },
